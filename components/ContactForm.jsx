    ...
    <Card>
      <form onSubmit={onSubmit} className="p-6 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <p className="text-xs text-gray-500">En envoyant, vous acceptez d’être recontacté(e).</p>
          <Button type="submit" variant="primary">
            <Send className="w-4 h-4"/>{status === "loading" ? " Envoi…" : status === "done" ? " Envoyé" : " Envoyer"}
          </Button>
        </div>
      </form>
    </Card>
...
export function Field({ label, name, type = "text", textarea, rows = 3, children, required, placeholder }){
  return (
